...
impl<'a> ContentInfo<'a> {
    pub fn tst_info(&self) -> Result<TSTInfo<'a>, asn1::ParseError> {
        ...
        let content_bytes = self.content.ok_or_else(|| {
            asn1::ParseError::new(asn1::ParseErrorKind::InvalidValue)
        })?;
...
mod tests {
    ...
    fn test_timestamp_fields() {
        ...
        let general_name = asn1::parse_single::<name::GeneralNameWrapper>(&enc_seq_general_name).unwrap();
        ...
        let content_info = asn1::parse_single::<ContentInfo>(&enc_content_info).unwrap();
    }
    ...
    fn test_parse_timestamp_response() {
        ...
        match tst_info_content {
            Content::SignedData(signed_data) => {
                ...
                assert_eq!(tst_info.version, 1);
                assert_eq!(tst_info.nonce.expect("Nonce must be set").as_bytes(), hex::decode("0B7DF056EDDA4995").unwrap());
            }
        }
    }
}